...
class XMLElementTests: XCTestCase {
	...
	func testParse() {
		guard let document = DataToXMLElementFactory(data: XMLElementTests.testXml).documentElement else {
			XCTFail()
			return
		}
		guard let firstChild = document.children.first as? SwiftPatterns.XMLElement
